...
def latticePlanner(ref_path,lidarObstacle,vehicle_status,current_lane):  #
    out_path=[]
    selected_lane=-1
    is_Obstacle = Float64MultiArray()
    is_Obstacle = lidarObstacle
    lattic_current_lane=current_lane
    look_distance=int(vehicle_status[3]*3.6*0.2*2) #k/h
    # print("\nlook_dist :\n", look_distance)
    if look_distance < 5 :
    ...
    if len(ref_path.poses)>look_distance :
        ...
        lane_off_set=[1.5,0.7,0.35,0,-0.35,-0.7,-1.5]
        ...
        global lane_weight
        global counting
        # if lattic_current_lane == 3:
        #     if(is_Obstacle.data[0]==1):     
        #         lane_weight = [106,104,0,100,102,104,106]
        #         counting = 0
        #         print("lane 1 !!Obstacle!!")
        #     else:
        #         lane_weight = [10,4,2,0,102,104,106]
        #         counting = 0
        #         print("No Obstacle")
        # elif lattic_current_lane == 2 and is_Obstacle.data[1] == 0: #and is_Obstacle.data[2]==0:
        #     counting = 0
        #     lane_weight = [10,4,2,0,102,104,106]
        #     print("back to lane 3")
        print("Vehicle_Y : ", vehicle_status[1])
        if (current_lane == 3):
            lane_weight = [106,104,2,0,102,104,106]
            if(-5.55 <= vehicle_status[1] <= -5.35):
                if(is_Obstacle.data[0]==1):     
                    lane_weight = [106,104,0,100,102,104,106]
                    print("lane 3 : !!Obstacle!! go to ", current_lane)
                else:
                    lane_weight = [10,4,2,0,102,104,106]
                    print("lane 3 : No Obstacle")
      
        elif( -5.30 <= vehicle_status[1] <= -5.0 and current_lane == 2 ):
            if(is_Obstacle.data[2] == 0):
                lane_weight = [10,4,2,0,102,104,106]
                print("lane 2 : No Obstacle")
            else:
                lane_weight = [106,104,0,100,102,104,106]
                print("lane 2 : !!Obstacle!!")
        

        # elif(current_lane == 3 and vision_lane == 2):
        #     lane_weight = [10,4,2,0,102,104,106]

        # elif(current_lane == 2 and vision_lane == 3):
                #     lane_weight = [106,104,0,100,102,104,106]
